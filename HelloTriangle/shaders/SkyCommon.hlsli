// ============================================================
// SkyCommon.hlsli
// Pure-function form of the procedural sky radiance.
// Shared by the rasterized sky dome (skyShaders.hlsl) and the IBL sky-capture
// compute shader (SkyCaptureCS.hlsl). Both MUST stay in sync — if they drift
// apart, the IBL ambient lighting stops matching the sky the player sees
// overhead. Keep all sky-radiance logic here; skyPS and the capture shader
// should only assemble a SkyParams struct and call EvalSkyRadiance().
//
// Takes only a world-space view direction plus sky parameters — no
// screen-space / SV_Position state, so it can be evaluated for arbitrary
// directions (cubemap texels) just as well as for dome pixels.
// ============================================================
...
struct SkyParams
{
    ...
    float3 middleColor;
    float3 bottomColor;
    float3 sunPosition;       // sun direction (normalized), encodes elevation via .y
    float3 sunColor;
    float3 moonPosition;      // moon direction (normalized)
    float3 moonCrescentDir;   // slowly-rotating crescent occlusion direction
...
float3 sky_renderStars(float3 dir, float time, float nightFactor)
{
    ...
    float h = sky_hash(cellId);
    if (h < 0.94) return float3(0, 0, 0); // ~6% of cells have a star

    // Star position within the cell
    float3 starOffset = float3(
        sky_hash(cellId + float3(1.3, 7.7, 2.5)),
        sky_hash(cellId + float3(9.1, 4.3, 6.7)),
        sky_hash(cellId + float3(3.7, 11.1, 8.3))
    );
    ...
    float colorVar = sky_hash(cellId + float3(5.5, 3.3, 9.9));
...
// Evaluate the full procedural sky radiance for a given world-space view
// direction (does not need to be normalized on input — normalized internally
// to mirror skyPS's normalize(input.worldPos)).
float3 EvalSkyRadiance(float3 dir, SkyParams p)
{
    float3 normalizedPos = normalize(dir);
    ...
    if (h > 0.55f)
    {
        ...
        skyColor = lerp(p.middleColor, p.topColor, t);
    }
    else
    {
        float t = smoothstep(0.0f, 0.55f, h);
        skyColor = lerp(p.bottomColor, p.middleColor, t);
    }

    // 2. Darken the nighttime sky (process before sun/moon so they are unaffected)
    float nightSky = saturate(-p.sunPosition.y * 3.0f);
    skyColor = lerp(skyColor, float3(0.02f, 0.02f, 0.08f), nightSky * 0.8f);

    // 2b. Stars (gradually appear at night; clouds will naturally cover them in subsequent steps)
    skyColor += sky_renderStars(normalizedPos, p.time, nightSky);

    // 2c. Lightning: sky flashes blue-white with high-frequency flicker
    float flicker = p.lightningIntensity * (0.8 + 0.2 * sin(p.time * 50.0));
    ...
    float sunVis  = smoothstep(-0.04, 0.06, p.sunPosition.y);
    // Delayed moon rise: moon starts appearing only after it climbs above 0.08 (~4.6 deg).
    // This creates a brief twilight gap after sunset so the moon does not overlap
    // with the rainbow's post-rain fade-out window (rainbow vanishes at sunDir.y ~-0.06).
    float moonVis = smoothstep(0.08, 0.18, p.moonPosition.y);

    // Additional suppression when camera is underwater
    float uwCamSky = saturate(-p.cameraY / 2.0);

    // 3. Sun radiance (composited after nighttime darkening)
    float3 sunDir = p.sunPosition.xyz;
    ...
    float3 crescentDir = normalize(moonDir - p.moonCrescentDir * p.crescentOffsetAmt);
    float crescentDot = max(0.0f, dot(normalizedPos, crescentDir));

    float moonBody    = pow(moonDot,    p.moonBodyPow);
    float moonOcclude = pow(crescentDot, p.moonOccludePow);
    float moonCrescent = saturate(moonBody - moonOcclude * 2.0f);
    skyColor += float3(1.0f, 1.0f, 0.95f) * moonCrescent * 18.0f * moonVis;

    // Halo
    float moonHalo = pow(moonDot, 100.0f) * 1.2f;
    skyColor += float3(0.3f, 0.4f, 0.8f) * moonHalo * moonVis;
    ...
    if (uwCamSky > 0.001)
    {
        ...
        skyColor = lerp(skyColor, deepColor, lookDown * uwCamSky);
        ...
        skyColor = lerp(skyColor,
            skyColor * float3(0.35, 0.70, 1.00),
        ...
        skyColor += float3(0.04, 0.18, 0.38) * horizon;
    }
