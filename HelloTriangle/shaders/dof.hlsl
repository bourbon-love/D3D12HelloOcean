...
// Achieves a bokeh blur effect via Vogel disk sampling (20 points).
// ============================================================
...
{
    float focusDepth;   // NDC depth of the focus plane (0=near, 1=far)
    float focusRange;   // half-width of in-focus region in NDC units
    float maxRadius;    // maximum CoC radius in UV space
...
float4 DOFPS(VSOut i) : SV_Target
{
    ...
    // Early return for fully in-focus pixels
    if (coc < 0.0005)
        return g_scene.SampleLevel(g_sampler, uv, 0);

    // Vogel disk sampling (golden-angle spiral) — 20 samples
    ...
    // Blend with center sample based on CoC amount (preserve sharp center)
